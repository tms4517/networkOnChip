// Testbench: NoC integration — 2 initiators / 2 targets
//
// Drives APB transactions through TWO niApbInitiator masters (on different
// routers) that both target TWO niApbTarget slaves (on different routers).
// Each initiator runs its own independent APB driver state machine so requests
...
#include "Vtb_nocApbIntegration1_top.h"
#include <verilated.h>
...
using Dut = Vtb_nocApbIntegration1_top;
...
int main(int argc, char **argv, char **env) {
  ...
  dut->trace(m_trace, 5);
  m_trace->open("waveform_nocApbIntegration1.vcd");
  ...
  std::cout << "=== NoC integration test: 2 initiators / 2 targets ==="
            << std::endl;
