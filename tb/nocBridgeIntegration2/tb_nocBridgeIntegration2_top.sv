...
// AHB-Lite peripheral.  Each initiator packs its NATIVE payload and its packet
// is tagged with the initiator protocol opcode.  The peripheral node
// (ahbTargetBridge) decodes the opcode, steers the packet to the matching
// per-protocol target NI, and converts it to AHB via the AXI-Lite->AHB /
// APB->AHB bridges (AHB is native and needs no bridge).
//
//   CPU  (AXI4-Lite) : (0, 0)
//   DMA  (AHB-Lite)  : (GRID_WIDTH-1, GRID_WIDTH-1)
//   IO   (APB)       : (GRID_WIDTH-1, 0)
//   AHB peripheral   : (0, GRID_WIDTH-1)
//   Address map (all initiators): 0x0000_0000-0x0FFF_FFFF -> peripheral

`default_nettype none

module tb_nocBridgeIntegration2_top
...
, output var logic [31:0] o_apb_prdata
  ...
  // {{{ Peripheral — ahbTargetBridge + AHB slave
  ...
  logic        per_hsel;  logic [31:0] per_haddr; logic per_hwrite;
  logic [2:0]  per_hsize; logic [1:0]  per_htrans; logic [31:0] per_hwdata;
  logic [31:0] per_hrdata; logic per_hready, per_hresp;
  ...
  , .o_hsel (per_hsel), .o_haddr (per_haddr), .o_hwrite (per_hwrite)
  , .o_hsize (per_hsize), .o_htrans (per_htrans), .o_hwdata (per_hwdata)
  , .i_hrdata (per_hrdata), .i_hready (per_hready), .i_hresp (per_hresp)
  );

  // Simple zero-wait AHB-Lite slave: 4 word registers.
  logic [31:0] slave_reg [0:3];
  logic        dphase_wr;
  logic [1:0]  dphase_addr;

  always_comb per_hready = 1'b1;
  always_comb per_hresp  = 1'b0;
  always_comb per_hrdata = slave_reg[dphase_addr];
  ...
  always_ff @(posedge i_clk or negedge i_arst_n)
    if (!i_arst_n) begin
      ...
      if (dphase_wr)
        slave_reg[dphase_addr] <= per_hwdata;
      if (per_hsel && per_htrans[1]) begin
        dphase_addr <= per_haddr[3:2];
        dphase_wr   <= per_hwrite;
      end else begin
        dphase_wr   <= 1'b0;
      end
    end
