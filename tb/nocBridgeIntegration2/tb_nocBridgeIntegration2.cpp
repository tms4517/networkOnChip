// Testbench: nocBridgeIntegration2
// A CPU (AXI4-Lite), a DMA (AHB-Lite) and an IO block (APB) all access one AHB
// peripheral through the NoC.  Packets are opcode-tagged; the peripheral node
// converts to AHB via the AXI-Lite->AHB / APB->AHB bridges (AHB native).
...
#include "Vtb_nocBridgeIntegration2_top.h"
#include <verilated.h>
...
using Dut = Vtb_nocBridgeIntegration2_top;
...
int main(int argc, char **argv, char **env) {
  ...
  dut->trace(m_trace, 5);
  m_trace->open("waveform_nocBridgeIntegration2.vcd");
  ...
  std::cout << "=== NoC bridge integration: AXI CPU + AHB DMA + APB IO -> AHB peripheral ==="
            << std::endl;
  ...
  // DMA (AHB, native — no bridge) -> AHB peripheral
