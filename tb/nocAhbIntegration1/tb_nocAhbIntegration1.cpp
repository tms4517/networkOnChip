// Testbench: NoC integration — 2 initiators / 2 targets (AHB-Lite)
//
// Drives AHB-Lite transactions through TWO niAhbInitiator masters (on different
// routers) that both target TWO niAhbTarget slaves (on different routers).
// Each initiator runs its own independent AHB driver state machine so requests
// from both initiators are in flight concurrently, exercising multi-initiator
// arbitration and dynamic source-based response routing.
//
...
#include "Vtb_nocAhbIntegration1_top.h"
#include <verilated.h>
...
using Dut = Vtb_nocAhbIntegration1_top;
...
int main(int argc, char **argv, char **env) {
  ...
  dut->trace(m_trace, 5);
  m_trace->open("waveform_nocAhbIntegration1.vcd");

  // Initiator 0: reads baseline of both targets, then writes/reads reg1.
  Driver d0;
  driverInit(d0, 0, {
  ...
  // Initiator 1: reads baseline of both targets, then writes/reads reg2.
  Driver d1;
  driverInit(d1, 1, {
    {0x00000000u, 0,           false, 0xA0000000u}, // read  TgtA reg0
    {0x10000000u, 0,           false, 0xB0000000u}, // read  TgtB reg0
  ...
  std::cout << "=== NoC AHB integration test: 2 initiators / 2 targets ==="
            << std::endl;
