// Testbench: nocBridgeIntegration1
// A CPU (AXI4-Lite) and a DMA (AHB-Lite) both access one APB peripheral through
// the NoC.  Packets are opcode-tagged; the peripheral node decodes the opcode
// and converts to APB via the AXI-Lite->APB / AHB->APB bridges.  Verifies data
// integrity and cross-initiator sharing of the peripheral.
...
#include "Vtb_nocBridgeIntegration1_top.h"
#include <verilated.h>
...
using Dut = Vtb_nocBridgeIntegration1_top;
...
int main(int argc, char **argv, char **env) {
  ...
  dut->trace(m_trace, 5);
  m_trace->open("waveform_nocBridgeIntegration1.vcd");
  ...
  std::cout << "=== NoC bridge integration: AXI CPU + AHB DMA + APB IO -> APB peripheral ==="
            << std::endl;
  ...
  // IO (APB, native — no bridge) -> APB peripheral
