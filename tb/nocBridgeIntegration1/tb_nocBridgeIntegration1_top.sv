...
// APB peripheral.  Each initiator packs its NATIVE payload and its packet is
// tagged with the initiator protocol opcode (top PROTOCOL_WIDTH bits, carried by
// widening the fabric PAYLOAD_WIDTH).  The peripheral node (apbTargetBridge)
// decodes the opcode, steers the packet to the matching per-protocol target NI,
// and converts it to APB via the AXI-Lite->APB / AHB->APB bridges (APB is native
// and needs no bridge).
//
//   CPU  (AXI4-Lite) : (0, 0)
//   DMA  (AHB-Lite)  : (GRID_WIDTH-1, GRID_WIDTH-1)
//   IO   (APB)       : (GRID_WIDTH-1, 0)
//   APB peripheral   : (0, GRID_WIDTH-1)
//   Address map (all initiators): 0x0000_0000-0x0FFF_FFFF -> peripheral

`default_nettype none

module tb_nocBridgeIntegration1_top
...
, localparam int unsigned COORD_WIDTH  = $clog2(GRID_WIDTH)
  // Fabric payload width with headroom for a future wider protocol; each NI
  // zero-pads its native payload (AXI/AHB = 71 bits) up to this width.
...
, output var logic [31:0] o_apb_prdata
  ...
  // {{{ Address map (both initiators reach the peripheral)
  ...
  // {{{ Peripheral — apbTargetBridge + APB slave
  ...
  logic [31:0] per_paddr, per_pwdata; logic per_pwrite;
  logic [3:0]  per_pstrb; logic per_psel, per_penable;
  logic        per_pready, per_pslverr; logic [31:0] per_prdata;
  ...
  , .o_paddr (per_paddr), .o_pwdata (per_pwdata), .o_pwrite (per_pwrite)
  , .o_pstrb (per_pstrb), .o_psel (per_psel), .o_penable (per_penable)
  , .i_pready (per_pready), .i_pslverr (per_pslverr), .i_prdata (per_prdata)
  );

  // Simple zero-wait APB slave: 4 word registers.
  logic [31:0] slave_reg [0:3];

  always_comb per_pslverr = 1'b0;

  always_comb per_pready = per_psel && per_penable;

  always_comb per_prdata = slave_reg[per_paddr[3:2]];
  ...
  always_ff @(posedge i_clk or negedge i_arst_n)
    ...
    end else if (per_psel && per_penable && per_pwrite) begin
      slave_reg[per_paddr[3:2]] <= per_pwdata;
    end
  // }}} Peripheral
